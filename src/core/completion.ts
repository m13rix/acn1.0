...
export const COMPLETION_SIGNAL_START = '__TELOS_TASK_DONE_START__';
export const COMPLETION_SIGNAL_END = '__TELOS_TASK_DONE_END__';

export const COMPLETION_SIGNAL_REGEX =
  /__(?:TELOS_TASK_DONE_START|TELOS_FINISH_START)__(.*?)__(?:TELOS_TASK_DONE_END|TELOS_FINISH_END)__/s;
...
export function buildCompletionWarning(): string {
  return `SYSTEM: Keep working until the task is truly complete. If you need clarification from the user, ask via \`message.ask(...)\` inside \`action(...)\`. Call \`${PRIMARY_COMPLETION_FUNCTION}("final user-facing message")\` only when you are done and ready to send the final result to the user.`;
}
