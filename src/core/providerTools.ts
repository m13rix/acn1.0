...
export function buildProviderToolRequest(requireFinish = true): ProviderToolRequest {
  const tools: ProviderToolRequest['tools'] = [
    ...
    },
    {
      ...
      function: {
        name: 'view_file',
        description: 'Read and return the contents of a single file by filename. Use "filename".',
        parameters: {
          type: 'object',
          properties: {
            filename: { type: 'string', description: 'Target file path, e.g. ./src/app.ts' },
