...
class StreamDisplay {
  ...
  writeReasoning(delta: string): void {
    // Truncate reasoning display to keep it manageable
    if (this.reasoningLineCount < 3) {
      ...
      for (const line of lines) {
        ...
        process.stdout.write(COLORS.muted(line));
        if (line !== lines[lines.length - 1]) {
          this.reasoningLineCount++;
        }
      }
    }
  }
...
async function runChat(session: Session): Promise<void> {
  ...
  /**
   * Pre-embed a word in real-time (fires and forgets)
   */
  const preEmbedWord = (word: string): void => {
    if (session.skillsService && word.length >= 2) {
      session.skillsService.embedWordRealtime(word);
      embeddedWordCount++;
    }
  };
  ...
  if (canUseRawMode) {
    ...
    process.stdin.on('data', async (chunk: string) => {
      ...
      if (chunk === '\r' || chunk === '\n') {
        ...
        isProcessing = true;
        try {
          if (debugMode && embeddedWordCount > 0) {
            console.log(COLORS.muted(`  Pre-embedded ${embeddedWordCount} words`));
          }
    ...
    const prompt = (): void => {
      rl.question(COLORS.primary(`${SYMBOLS.user} `), async (input) => {
        ...
        // Pre-embed all words before execution (for non-TTY mode)
        if (session.skillsService) {
          const words = extractWords(trimmed);
          await session.skillsService.embedWordsRealtime(words);
        }
...
async function main(): Promise<void> {
  ...
  if (!process.env['GEMINI_API_KEY'] && !process.env['OPENROUTER_API_KEY']) {
    ...
    console.log(COLORS.muted('Set GEMINI_API_KEY or OPENROUTER_API_KEY in your environment.\n'));
  }
