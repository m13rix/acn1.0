// Exa tool - Web search with Exa Answer API and Exa Search API
 ...
 * Similar to what would be passed to Google models
 */
function formatSearchResults(searchResult, query) {
    ...
    searchResult.results.forEach((result, index) => {
        ...
        if (result.text) {
            // Truncate text to reasonable length (first 500 chars)
...
/**
 * Perform web search using Exa API and return formatted results
 */
async function performWebSearch(query, ui = null) {
    try {
        console.log(chalk.blue(`\n🌐 Веб-поиск через Exa Search API: "${query}"`));

        if (ui) {
            ui.update({ data: {
                query,
                engine: 'Exa Search API',
        ...
        console.log(chalk.gray('📝 Форматирование результатов...'));
        const formattedResults = formatSearchResults(searchResult, query);
        ...
        console.error(chalk.red('❌ Ошибка Web Search (Exa):'), error.message);
        throw error;
    }
}
...
function createExaTool(toolUIManager) {
    return {
        getAnswer: async (query) => {
            console.log(chalk.blue('\n🔍 Выполняется операция: exa.getAnswer'));
            ...
            try {
                ...
                if (ui) ui.finish();
                console.error(chalk.red('❌ Ошибка в exa.getAnswer:'), error.message);
                return `Ошибка: ${error.message}`;
            }
        },

        webSearch: async (query) => {
            console.log(chalk.blue('\n🌐 Выполняется операция: exa.webSearch'));
            if (!query) throw new Error('Требуется параметр query');
            ...
            try {
                ...
                if (ui) ui.finish();
                console.error(chalk.red('❌ Ошибка в exa.webSearch:'), error.message);
                return `Ошибка: ${error.message}`;
            }
        }
    };
...
export const exa = {
    _initUI: (toolUIManager) => createExaTool(toolUIManager),
    getAnswer: async (query) => {
        if (!query) throw new Error('Требуется параметр query');
