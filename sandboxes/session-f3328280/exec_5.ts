...
console.log("📚 Этап 1: Получение текста задания из учебника русского языка...");
...
await message.sendText(`✅ Текст упражнения 156 получен:\n\n${taskText}\n\nПроверь, всё ли правильно. Начинать выполнение?`);
})().catch(err => {
  console.error(err);
  process.exit(1);
}).then(() => {
