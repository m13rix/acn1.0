...
// Текст задания (пример)
const taskText = `В предложении: "Когда я пришёл домой, мама уже готовила ужин, и я сразу сел за стол."`;

// Передаем задание подагенту для выполнения
await agents.call(
  "russian_expert",
  `Ты получил задание по русскому языку (упражнение 156). Тебе нужно выполнить ВСЕ подзадачи этого упражнения одним решением. Вот полный текст задания:
