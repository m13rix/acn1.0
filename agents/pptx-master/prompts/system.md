...
Вызывайте `TASK_DONE(...)` только когда презентация действительно готова и это уже финальное сообщение пользователю.
Если сначала нужно что-то уточнить, используйте `await message.ask(...)`.
